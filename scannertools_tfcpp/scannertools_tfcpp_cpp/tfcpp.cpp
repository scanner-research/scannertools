...
class TFCKernelCPU : public BatchedKernel {
 public:
  TFCKernelCPU(const KernelConfig& config)
    : BatchedKernel(config), device_(config.devices[0]) {}

  void setup_with_resources(proto::Result* result) override {
    std::string graph_path = "/tmp/ssd_mobilenet_v1_coco_2017_11_17/frozen_inference_graph.pb";
  ...
  void execute(const BatchedElements& input_columns,
    ...
    TF_Output output_op = {TF_GraphOperationByName(graph_, "detection_boxes"), 0};
    ...
    int batch_size = input_columns[0].size();
    shape[0] = batch_size;
    for (size_t i = 0; i < FRAME_DIMS; ++i) { shape[i+1] = frame->shape[i]; }

    TF_Tensor* input_tensor =
      TF_NewTensor(frame_type_to_tf_type(frame->type), shape, FRAME_DIMS + 1, frame->data, frame->size() * batch_size,
                   free_tensor, nullptr);
...
REGISTER_KERNEL(TFC, TFCKernelCPU)
