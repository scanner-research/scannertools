from scannerpy.types import register_type, UniformList, NumpyArrayFloat32
...
class Pose:
    ...
    def __init__(self, score, kp):
        self._score = score
        self._kp = kp

    def serialize(buf):
        return NumpyArrayFloat32.serialize(buf._kp)

    def deserialize(buf):
        arr = NumpyArrayFloat32.deserialize(buf)
        score = arr[0]
        arr = arr[1:].reshape((Pose.POSE_KEYPOINTS + Pose.FACE_KEYPOINTS + Pose.HAND_KEYPOINTS * 2, 3))
        return Pose(score, arr)
    ...
    def pose_keypoints(self):
        return self._kpo[:self.POSE_KEYPOINTS, :]
    ...
    def hand_keypoints(self):
        base = self._kpo[self.POSE_KEYPOINTS + self.FACE_KEYPOINTS:, :]
    ...
    def draw(self, img, thickness=5, draw_threshold=0.05):
        ...
        def to_pt(i):
            ...
            if not check(self._kp[i, 0]) or not check(self._kp[i, 1]):
                return None
            return (int(self._kp[i, 0] * img.shape[1]),
                    int(self._kp[i, 1] * img.shape[0]))

        for ([a, b], color) in zip(self.DRAW_PAIRS, self.DRAW_COLORS):
            if self._kp[a, 2] > draw_threshold and \
               self._kp[b, 2] > draw_threshold:
                pt_a = to_pt(a)
                pt_b = to_pt(b)
...
PoseList = UniformList('PoseList', Pose, size=Pose.kp_size() * 4)
