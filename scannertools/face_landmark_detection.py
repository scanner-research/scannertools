...
@scannerpy.register_python_op(name='DetectFaceLandmarksCPU', device_type=DeviceType.CPU)
@scannerpy.register_python_op(name='DetectFaceLandmarksGPU', device_type=DeviceType.GPU)
    ...
    def execute(self, frame: FrameType, bboxes: bytes) -> bytes:
        ...
        predictions = self.fa.get_landmarks_from_image(
                ...
                (bbox.x1 * w,
                 bbox.y1 * h,
                 bbox.x2 * w,
                 bbox.y2 * h) for bbox in bboxes])

        predictions = [
            np.array([[width / w, height / h]
                for [width, height] in prediction])
