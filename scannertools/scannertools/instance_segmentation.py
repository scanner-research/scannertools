...
CONFIG_FILE = "https://github.com/facebookresearch/maskrcnn-benchmark/blob/master/configs/caffe2/e2e_mask_rcnn_X_101_32x8d_FPN_1x_caffe2.yaml"
...
class MaskRCNNSegmentObjects(Kernel):
    def __init__(self, 
        ...
        # set maskrcnn config
        config_file_path = download_temp_file(CONFIG_FILE)
    ...
    def build_transform(self):
        ...
        transform = T.Compose(
            [
                T.ToPILImage(),
                T.Resize(self.config.min_image_size),
                T.ToTensor(),
                to_bgr_transform,
                normalize_transform,
            ]
    ...
    def execute(self, frame: Sequence[FrameType]) -> Sequence[Any]:
        ...
        return [{'bbox': {'x1' : bbox[0], 'y1': bbox[1], 'x2' : bbox[2], 'y2' : bbox[3]},
                'mask' : mask, 'label' : label, 'score' : score}
...
def draw_boxes(config, frame: FrameType, bundled_data: bytes) -> FrameType:
    ...
    visualize_boxes_and_labels_on_image_array(frame, metadata, min_score_thresh)
    return frame
...
def visualize_boxes_and_labels_on_image_array(frame, metadata, min_score_thresh):
    labels = [obj['score'] for obj in metadata]
    ...
    for score, box, mask, label, color in zip(scores, boxes, masks, labels, colors):
        ...
        image = cv2.rectangle(
            image, top_left, bottom_right, tuple(color), 1
        )
